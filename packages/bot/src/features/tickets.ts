  ...
  const subject = new LabelBuilder()
    ...
    .setTextInputComponent(
      new TextInputBuilder()
        .setCustomId(TICKET_IDS.subject)
  ...
  const images = new LabelBuilder()
    ...
    .setFileUploadComponent(
      new FileUploadBuilder()
        .setCustomId(TICKET_IDS.images)
        .setMinValues(0)
        .setMaxValues(MAX_IMAGES)
        .setRequired(false),
    )
  return new ModalBuilder()
    .setCustomId(TICKET_IDS.modal)
    .setTitle('Raise a ticket')
    .addLabelComponents(subject, description, images)
}
...
function statusButtons(
  guildId: string,
  ticket: { id: number; status: 'open' | 'resolved' | 'closed'; threadId: string | null },
) {
  const threadBtn = ticket.threadId
    ? new ButtonBuilder()
        ...
        .setURL(`https://discord.com/channels/${guildId}/${ticket.threadId}`)
    : new ButtonBuilder()
...
function ticketEmbed(t: {
  ...
  return new EmbedBuilder()
    ...
    .addFields(
      { name: 'Raised by', value: `<@${t.userId}>`, inline: true },
      { name: 'Status', value: t.status, inline: true },
      { name: 'Images', value: String(t.imageCount), inline: true },
    )
...
export async function handleTicketButton(interaction: ButtonInteraction): Promise<void> {
  ...
  if (
    (action !== 'thread' && action !== 'resolve' && action !== 'close') ||
    !Number.isInteger(id)
  ) {
    return
  }
...
async function createThread(interaction: ButtonInteraction, id: number): Promise<void> {
  ...
  try {
    ...
    const threads = channel.threads as GuildTextThreadManager<
...
async function fetchImage(
  att: Attachment,
): Promise<{ data: Buffer; filename: string; contentType: string; sizeBytes: number } | null> {
...
export async function handleTicketModal(interaction: ModalSubmitInteraction): Promise<void> {
  ...
  try {
    ...
    const extra =
      skipped > 0
        ? ` (${skipped} file${skipped === 1 ? '' : 's'} skipped — images only, max 8MB)`
        : ''
    await interaction.editReply(
      `✅ Ticket **#${ticket.id}** submitted — our staff will review it shortly.${extra}`,
    )
  } catch (err) {
    log.error('tickets', `failed to post ticket #${ticket.id}: ${(err as Error).message}`)
    await interaction.editReply(
      `⚠️ Ticket **#${ticket.id}** was saved but couldn’t be posted to staff. A mod will need to check the bot’s access to the collection channel.`,
    )
  }
}
  ...
  for (const user of mentioned.values()) {
    ...
    if (authorIsStaff) {
      ...
      try {
        ...
        log.warn(
          'tickets',
          `failed to add <@${user.id}> to #${ticket.id}: ${(err as Error).message}`,
        )
      }
    } else {
