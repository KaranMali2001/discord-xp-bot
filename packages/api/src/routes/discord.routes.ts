...
export async function discordRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get<{ Params: GuildParams }>('/guilds/:guildId/discord/roles', async (request) => {
    return discordController.roles(request.params.guildId)
  })

  // Creating a role mutates the guild — guard it.
  app.post<{ Params: GuildParams }>(
    '/guilds/:guildId/discord/roles',
    { preHandler: requireManageParam },
    async (request) => {
      return discordController.createRole(request.params.guildId, request.body)
    },
  )

  app.get<{ Params: GuildParams }>('/guilds/:guildId/discord/members', async (request, reply) => {
